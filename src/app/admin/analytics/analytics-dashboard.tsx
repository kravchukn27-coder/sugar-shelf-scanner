...
export default function AnalyticsDashboard() {
  ...
  return <main className={styles.page}>
    ...
    {view === "gemini" && <section className={styles.geminiHealth} aria-label="Gemini Health">
      ...
      <details className={styles.widePanel} open={overview.geminiHealth.models.length > 1}><summary className={styles.panelHeading}><div><p className={styles.eyebrow}>Models</p><h2>Seven-day breakdown</h2></div><span>{overview.geminiHealth.models.length} seen</span></summary>{overview.geminiHealth.models.length === 0 ? <p className={styles.empty}>No Gemini provider events in this window yet.</p> : <div className={styles.modelTable}>{overview.geminiHealth.models.map((model) => <div key={model.model}><strong>{model.model}</strong><span>{model.requests} req · {formatPercent(model.requests ? model.errors / model.requests : null)} errors</span><span>p95 {model.p95LatencyMs === null ? "—" : `${Math.round(model.p95LatencyMs)}ms`} · {model.timeoutErrors} timeouts</span><span>{formatValue(model.totalTokens, "count")} tokens · {formatBilling(model.estimatedCostUsd, "USD")}</span></div>)}</div>}</details>
