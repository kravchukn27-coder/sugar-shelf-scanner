...
export default function AnalyticsDashboard() {
  ...
  return <main className={styles.page}>
    ...
    {view === "gemini" && <section className={styles.geminiHealth} aria-label="Gemini Health">
      ...
      })}{overview.geminiHealth.hedgeStats.map((stat) => <span key={stat.operation} className={styles.breakerBadgeClosed}>{formatOperationLabel(stat.operation)} hedge: won {stat.won}/{stat.eligible} ({formatPercent(stat.eligible ? stat.won / stat.eligible : null)})</span>)}{overview.geminiHealth.confidenceStats.total > 0 && <span className={styles.breakerBadgeClosed}>Model-confident: {formatPercent(modelConfidenceRate)} ({overview.geminiHealth.confidenceStats.total - overview.geminiHealth.confidenceStats.defaultConfidenceCount}/{overview.geminiHealth.confidenceStats.total})</span>}</div>
