...
import { getSugarFitDemoScan, SUGAR_FIT_DEMO_IMAGE } from "@/lib/mock/scan-fixtures";
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("demo") !== "sugar-fit") return;
    setShowIntro(false);
    setScan(getSugarFitDemoScan());
    setFrozen(SUGAR_FIT_DEMO_IMAGE);
    setState("results");
    setSelected("demo-corona-extra");
    setSheet(true);
  }, []);
  ...
  const singleResultId = state === "results" && groups.length === 1 ? groups[0]?.detection.id ?? null : null;
  useEffect(() => { if (singleResultId) { setSelected(singleResultId); setSheet(true); } }, [singleResultId]);
  ...
  return <>{showIntro && !recoveryActive && <OnboardingStory onFinish={() => setShowIntro(false)} />}<main className="scanner-shell"><section className={`camera-scene ${state === "camera_off" ? "idle" : ""} ${recoveryActive ? "recovery-active" : ""}`} aria-label={recoveryActive ? "Recovery camera" : "Sugar product scanner"}>
    ...
    {state === "captured_analyzing" && <CameraCopy>{analysisPhase === "identifying" ? "Product found — calculating your Sugar Fit…" : analysisPhase === "catalog" ? "Personalizing your result…" : "Taking a little longer than usual…"}</CameraCopy>}
  ...
  </section>{state === "results" && !recoveryActive && <SugarFitResultHandle groups={groups} frozenImage={frozen} onOpen={() => setSheet(true)} />}{sheet && !recoveryActive && <SugarFitResultsSheet groups={groups} frozenImage={frozen} selectedId={selected} recovery={recovery} recoveryBanner={recoverySubmissionBanner} onRecover={startRecovery} onSelect={setSelected} onClose={() => { setSheet(false); setSelected(null); }} onScanAgain={() => void start()} />}<canvas ref={canvasRef} className="hidden-canvas" /></main></>;
}
