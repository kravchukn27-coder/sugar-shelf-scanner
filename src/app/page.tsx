...
// inside the capture rectangle; a skip/uncertain reason below
...
export default function HomePage() {
  ...
  return <>{introResolved && showIntro && !recoveryActive && <OnboardingStory onFinish={finishIntro} onStart={startFromIntro} />}{paywallEnabled && paywallOpen && !recoveryActive && <Paywall checkoutAvailable={checkoutUrl.length > 0} restoreState={restoreState} onCheckout={startCheckout} onRestore={(email) => void restoreAccess(email)} onClose={() => setPaywallOpen(false)} />}<main className="scanner-shell"><section className={`camera-scene ${state === "camera_off" ? "idle" : ""} ${recoveryActive ? "recovery-active" : ""}`} aria-label={recoveryActive ? "Recovery camera" : "Sugar product scanner"}>
    ...
    {viewfinderVisible && !frozen && !uploadUrl && !recoveryActive && <div className={`camera-live-preview ${canvasPreviewActive ? "active" : ""}`}><canvas ref={liveCanvasRef} className="camera-canvas-foreground" aria-hidden="true" /><div ref={livePreviewRef} className="camera-capture-frame">{!failed && <p className="live-hint" aria-live="polite">{liveHint ?? LIVE_HINT_DEFAULT}</p>}</div></div>}
    {frozen && !recoveryActive && <>{!uploadUrl && <div className="camera-result-preview"><img className="frozen-preview camera-result-image" src={frozen} alt="Captured products" /></div>}{uploadUrl && <img className="camera-preview frozen-preview" src={frozen} alt="Captured products" />}</>}{state !== "camera_off" && <div className="camera-vignette" />}
    ...
    {state === "live_searching" && !uploadUrl ? <button className="analyze-now-button" type="button" aria-label="Tap to analyze" disabled={!canvasPreviewActive} onClick={analyzeNow}><span className="analyze-now-shutter" aria-hidden="true" /><span>Tap to analyze</span></button> : null}
